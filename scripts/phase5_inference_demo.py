"""
Phase 5: Interactive Inference Demo
...
Requires Phase 4 (FAISS index build) to have been run first.

Usage:
    python scripts/phase5_inference_demo.py --pipeline A
    python scripts/phase5_inference_demo.py --pipeline B --rewrite
"""
import sys
import time
import argparse
...
def main():
    ...
    if not faiss_path.exists():
        logger.error("FAISS index not found at %s. Run phase4_build_index.py first.", faiss_path)
        sys.exit(1)
